...
namespace CoordinateTransformer
{
    ...
    void CoordinateTransformer::setBounds(const std::string &frame_id, const geometry_msgs::msg::Point &min, const geometry_msgs::msg::Point &max)
    {
        ...
        if (min.x > max.x)
        {
            throw Exceptions::InvertedBoundsException("X");
        }
        if (min.y > max.y)
        {
            throw Exceptions::InvertedBoundsException("Y");
        }
        if (min.z > max.z)
        {
            throw Exceptions::InvertedBoundsException("Z");
        }
        ...
        {
            throw Exceptions::EqualBoundsException();
        }
    }

    void CoordinateTransformer::CoordinateTransformer::loadConfig(const std::string &config_file)
    {
        YAML::Node config;
        try
        {
        ...
        catch (const YAML::BadFile &ex)
        {
            ...
            throw Exceptions::NoFileException(config_file);
            return;
        }
        ...
        if (config["transforms"])
        {
            for (const auto &transform : config["transforms"])
            {
                ...
                catch (const YAML::Exception &ex)
                {
                    ...
                    throw Exceptions::ParsedBadTransformException();
                    continue;
                }
            }
        }
        ...
        {
            throw Exceptions::NoTransformsFoundException();
            return;
        }
        ...
        if (config["bounds"])
        {
            for (const auto &bound : config["bounds"])
            {
                ...
                catch (const YAML::Exception &ex)
                {
                    ...
                    throw Exceptions::ParsedBadBoundsException();
                    continue; // Skip this bound but continue with others
                }
                catch (const Exceptions::BoundValidationException &ex)
                {
                    RCLCPP_ERROR(
                        rclcpp::get_logger("CoordinateTransformer"),
                        "Invalid bounds configuration: %s", ex.what());
                    throw Exceptions::ParsedInvalidBoundsException();
                    continue;
                }
            }
        }
